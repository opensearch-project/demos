...
class ClusterBootstrap:
  ...
  def _initialize_cluster_settings(self):
    ...
    data = {
      ...
      "plugins.ml_commons.only_run_on_ml_node":
        False,
      "plugins.ml_commons.connector_access_control_enabled":
        True,
      "plugins.ml_commons.model_access_control_enabled":
        True,
      "plugins.ml_commons.memory_feature_enabled":
        True,
      "plugins.ml_commons.rag_pipeline_feature_enabled":
        True,
      "plugins.ml_commons.trusted_connector_endpoints_regex": [
        "^https://runtime\\.sagemaker\\..*[a-z0-9-]\\.amazonaws\\.com/.*$",
        "^https://api\\.openai\\.com/.*$", "^https://api\\.cohere\\.ai/.*$"
      ]
    }
  ...
  def _init_index_template(self, template_name="nlp-template"):
    ...
    if not self.client._client.indices.exists_template(name=template_name):
      ...
      response = self.client._client.indices.put_template(name=template_name, body=template)
  ...
  def _initialize_connector(self, connector_name: str = "Cohere Connector"):
    ...
    connector_data = {
      "name":
        connector_name,
      "description":
        "External connector for connections into Cohere",
      "version":
        "1.0",
      "protocol":
        "http",
      ...
      "actions": [{
        ...
        "request_body":
          "{ \"texts\": ${parameters.prompt}, \"truncate\": \"${parameters.truncate}\", \"model\": \"${parameters.model}\" }",
        "pre_process_function":
          "connector.pre_process.cohere.embedding",
        "post_process_function":
          "connector.post_process.cohere.embedding"
      }]
    }

    connector_id = self.client.get_connector_id(connector_name=connector_name)
  ...
  def _initialize_model(self, model_name: str, model_descp: str):
    ...
    response = self.client.register_connector_model(model_meta_json=model_meta)
    if isinstance(response, str):
  ...
  def ml_cleanup(self):
    try:
      if self.embedding_model:
        undeploy_model_response = self.client.undeploy_model(self.embedding_model)
      ...
      if self.connector_id:
        delete_connector_response = self.client.delete_connector(self.connector_id)
      ...
      delete_index_response = self.client._client.indices.delete(index='cohere-index')
      if delete_index_response and 'acknowledged' in delete_index_response and delete_index_response[
          'acknowledged']:
        print('\nDeleting index:')
      ...
      if delete_pipeline_response and 'acknowledged' in delete_pipeline_response and delete_pipeline_response[
          'acknowledged']:
        print('\nDeleting pipeline:')
      ...
      if delete_search_pipeline_response and 'acknowledged' in delete_search_pipeline_response and delete_search_pipeline_response[
          'acknowledged']:
        print('\nDeleting pipeline:')
